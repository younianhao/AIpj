...
def split_and_combine_images(input_rgb_files,input_tir_files,input_xml_files, output_rgb_file, output_tir_file, output_xml_file):
    # 处理rgb图像文件
    ...
    # 处理tir图像文件
    img1 = Image.open(input_tir_files[0])
    img2 = Image.open(input_tir_files[1])
    img3 = Image.open(input_tir_files[2])
    img4 = Image.open(input_tir_files[3])
    part1 = img1.crop((0, 0, half_width, half_height))
    part2 = img2.crop((half_width, 0, width, half_height))
    part3 = img3.crop((0, half_height, half_width, height))
    part4 = img4.crop((half_width, half_height, width, height))
    ...
    combined_img.paste(part3, (0, half_height))
    combined_img.paste(part4, (half_width, half_height))
    combined_img.save(output_tir_file)
...
for index1, index2, index3, index4 in zip(selected_files1, selected_files2, selected_files3, selected_files4):
    ...
    input_tir_files = [os.path.join(tir_path, str(index1) + "R.jpg"), os.path.join(tir_path, str(index2) + "R.jpg"), os.path.join(tir_path, str(index3) + "R.jpg"), os.path.join(tir_path, str(index4) + "R.jpg")]
    ...
    output_tir_file = os.path.join(tir_path, "_combined" + str(index1) + "_" + str(index2) + "_" + str(index3) + "_" + str(index4) + "R.jpg")
    output_xml_file = os.path.join(xml_path, "_combined" + str(index1) + "_" + str(index2) + "_" + str(index3) + "_" + str(index4) + "R.xml")

    split_and_combine_images(input_rgb_files,input_tir_files,input_xml_files, output_rgb_file, output_tir_file, output_xml_file)
