...
def load_data(img_path, gt_path, train=True):
    ...
    new_height = max(1, target.shape[0] // 8)
    new_width = max(1, target.shape[1] // 8)
    target = cv2.resize(
        target, (new_width, new_height), interpolation=cv2.INTER_CUBIC) * 64
    return img, target
...
seed = time.time()
print_freq = 30
img_dir = "./dataset_out/train/resized_images/"
gt_dir = "./dataset_out/train/hdf5s/"
pre = None
task = ""
